...
export const fetchTools = () => {
    return (dispatch) => {
        ...
        .then(data => {
            ...
            const unavailableTools = toolsArray.filter(tool => tool.available !== true)
            const availableTools = toolsArray.filter(tool => tool.available !== false)
            // console.log("coming from toolsfetchaction",toolsArray)
            dispatch(loadMarked(unavailableTools))
            dispatch(loadTools(availableTools))
        })
    }
}
// --------------
